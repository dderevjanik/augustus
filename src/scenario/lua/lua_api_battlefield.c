...
// Helper: parse an army table from a Lua table at the given stack index
...
// battlefield.start(config?)
static int l_battlefield_start(lua_State *L)
{
...
// battlefield.start_from_map(filename, config?)
static int l_battlefield_start_from_map(lua_State *L)
{
...
// battlefield.stop()
static int l_battlefield_stop(lua_State *L)
{
    battlefield_stop();
    return 0;
}

// battlefield.is_active() -> boolean
static int l_battlefield_is_active(lua_State *L)
{
    lua_pushboolean(L, battlefield_is_active());
    return 1;
}

// battlefield.spawn_enemies(army_table) -> boolean
static int l_battlefield_spawn_enemies(lua_State *L)
{
    luaL_checktype(L, 1, LUA_TTABLE);
...
// battlefield.win()
static int l_battlefield_win(lua_State *L)
{
    battlefield_win();
    return 0;
}
...
// battlefield.lose()
static int l_battlefield_lose(lua_State *L)
{
    battlefield_lose();
    return 0;
}
...
{
    luaL_newlib(L, battlefield_funcs);

    // battlefield.FIGURE_TYPE.* constants
    lua_newtable(L);
    ...
    // battlefield.ENEMY_TYPE.* constants
    lua_newtable(L);
