...
void scenario_request_make_visible_and_send_message(int id);
        ...
        if (state == REQUEST_STATE_DISPATCHED || state == REQUEST_STATE_DISPATCHED_LATE) {
            ...
            if (scenario.requests[i].visible) {
                ...
                if (game_time_year() == year + scenario.requests[i].year &&
                    ...
                    scenario_request_make_visible_and_send_message(i);
                }
            }
        }
    }
...
int scenario_request_force_request_start(int id)
{
    if (!scenario.requests[id].resource) {
        return 0;
    }
    ...
    scenario_request_make_visible_and_send_message(id);
