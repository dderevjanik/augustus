...
static void xml_exporter_export_declaration_doctype(const char *type);
static void xml_exporter_finish_start_tag(int has_children);
static void xml_exporter_create_end_tag();

static int xml_exporter_same_depth_new_element();
static int xml_exporter_increase_depth();
static int xml_exporter_decrease_depth();
...
void xml_exporter_init(buffer *buf, const char *type)
{
    ...
    data.success = 0;
    xml_exporter_export_declaration_doctype(type);
}

void xml_exporter_new_element(const char *name, int is_child)
{
    if (is_child) {
        xml_exporter_finish_start_tag(1);
        xml_exporter_increase_depth();
    } else {
        xml_exporter_close_element();
        xml_exporter_same_depth_new_element();
    }
    ...
    xml_export_ascii_as_const_string(data.output_buf, data.current_element->name);
}

void xml_exporter_add_attribute_text(const char *name, const uint8_t *value)
{
    xml_exporter_whitespaces(1);
    xml_export_ascii_as_const_string(data.output_buf, name);
    char text_out[] = "=\"";
    xml_export_ascii_as_string(data.output_buf, text_out);
    buffer_write_raw(data.output_buf, value, string_length(value));
    char text_out_2[] = "\"";
    xml_export_ascii_as_string(data.output_buf, text_out_2);
}
...
static void xml_exporter_export_declaration_doctype(const char *type)
{
    ...
    xml_export_ascii_as_const_string(data.output_buf, type);
    buffer_write_raw(data.output_buf, ">", 1);
    xml_exporter_newline();
}
...
void xml_exporter_newline()
{
    xml_export_ascii_as_string(data.output_buf, "\n");
}
