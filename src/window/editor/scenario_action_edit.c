#include "scenario_action_edit.h"

#include "core/string.h"
#include "graphics/button.h"
#include "graphics/generic_button.h"
#include "graphics/graphics.h"
#include "graphics/lang_text.h"
#include "graphics/panel.h"
#include "graphics/screen.h"
#include "graphics/text.h"
#include "graphics/window.h"
#include "input/input.h"
#include "scenario/scenario_events_xml.h"
#include "window/editor/map.h"
#include "window/numeric_input.h"

#define MAX_BUTTONS 5
#define BUTTON_LEFT_PADDING 32
#define BUTTON_WIDTH 320
#define DETAILS_Y_OFFSET 128
#define DETAILS_ROW_HEIGHT 32

static void init(scenario_action_t *action);
static void button_amount(int param1, int param2);

static generic_button buttons[] = {
    {BUTTON_LEFT_PADDING, DETAILS_Y_OFFSET + (0 * DETAILS_ROW_HEIGHT), BUTTON_WIDTH, DETAILS_ROW_HEIGHT - 2, button_amount, button_none, 1, 0},
    {BUTTON_LEFT_PADDING, DETAILS_Y_OFFSET + (1 * DETAILS_ROW_HEIGHT), BUTTON_WIDTH, DETAILS_ROW_HEIGHT - 2, button_amount, button_none, 2, 0},
    {BUTTON_LEFT_PADDING, DETAILS_Y_OFFSET + (2 * DETAILS_ROW_HEIGHT), BUTTON_WIDTH, DETAILS_ROW_HEIGHT - 2, button_amount, button_none, 3, 0},
    {BUTTON_LEFT_PADDING, DETAILS_Y_OFFSET + (3 * DETAILS_ROW_HEIGHT), BUTTON_WIDTH, DETAILS_ROW_HEIGHT - 2, button_amount, button_none, 4, 0},
    {BUTTON_LEFT_PADDING, DETAILS_Y_OFFSET + (4 * DETAILS_ROW_HEIGHT), BUTTON_WIDTH, DETAILS_ROW_HEIGHT - 2, button_amount, button_none, 5, 0}
};

static struct {
    int focus_button_id;

    scenario_action_t *action;
    scenario_action_data_t *xml_info;
} data;

static void init(scenario_action_t *action)
{
    data.action = action;
    data.xml_info = scenario_actions_get_xml_attributes(action->type);
}

static void draw_background(void)
{
    window_editor_map_draw_all();
}

static void draw_foreground(void)
{
    graphics_in_dialog();

    outer_panel_draw(16, 16, 24, 24);

    text_draw_centered(translation_for(TR_EDITOR_SCENARIO_ACTION), 32, 32, BUTTON_WIDTH, FONT_LARGE_BLACK, 0);
    text_draw_centered(string_from_ascii(data.xml_info->xml_attr.name), 32, 64, BUTTON_WIDTH, FONT_LARGE_BLACK, 0);

    int y_offset = DETAILS_Y_OFFSET;
    int button_id = 0;
    if (data.xml_info->xml_parm1.type > PARAMETER_TYPE_UNDEFINED) {
        large_label_draw(buttons[button_id].x, buttons[button_id].y, buttons[button_id].width / 16, data.focus_button_id == button_id + 1 ? 1 : 0);
        text_draw_label_and_number(string_from_ascii(data.xml_info->xml_parm1.name), data.action->parameter1, "", 64, y_offset + 8, FONT_NORMAL_PLAIN, COLOR_BLACK);
    }
    y_offset += DETAILS_ROW_HEIGHT;
    button_id++;

    if (data.xml_info->xml_parm2.type > PARAMETER_TYPE_UNDEFINED) {
        large_label_draw(buttons[button_id].x, buttons[button_id].y, buttons[button_id].width / 16, data.focus_button_id == button_id + 1 ? 1 : 0);
        text_draw_label_and_number(string_from_ascii(data.xml_info->xml_parm2.name), data.action->parameter2, "", 64, y_offset + 8, FONT_NORMAL_PLAIN, COLOR_BLACK);
    }
    y_offset += DETAILS_ROW_HEIGHT;
    button_id++;

    if (data.xml_info->xml_parm3.type > PARAMETER_TYPE_UNDEFINED) {
        large_label_draw(buttons[button_id].x, buttons[button_id].y, buttons[button_id].width / 16, data.focus_button_id == button_id + 1 ? 1 : 0);
        text_draw_label_and_number(string_from_ascii(data.xml_info->xml_parm3.name), data.action->parameter3, "", 64, y_offset + 8, FONT_NORMAL_PLAIN, COLOR_BLACK);
    }
    y_offset += DETAILS_ROW_HEIGHT;
    button_id++;

    if (data.xml_info->xml_parm4.type > PARAMETER_TYPE_UNDEFINED) {
        large_label_draw(buttons[button_id].x, buttons[button_id].y, buttons[button_id].width / 16, data.focus_button_id == button_id + 1 ? 1 : 0);
        text_draw_label_and_number(string_from_ascii(data.xml_info->xml_parm4.name), data.action->parameter4, "", 64, y_offset + 8, FONT_NORMAL_PLAIN, COLOR_BLACK);
    }
    y_offset += DETAILS_ROW_HEIGHT;
    button_id++;

    if (data.xml_info->xml_parm5.type > PARAMETER_TYPE_UNDEFINED) {
        large_label_draw(buttons[button_id].x, buttons[button_id].y, buttons[button_id].width / 16, data.focus_button_id == button_id + 1 ? 1 : 0);
        text_draw_label_and_number(string_from_ascii(data.xml_info->xml_parm5.name), data.action->parameter5, "", 64, y_offset + 8, FONT_NORMAL_PLAIN, COLOR_BLACK);
    }
    y_offset += DETAILS_ROW_HEIGHT;
    button_id++;

    lang_text_draw_centered(13, 3, 32, 32 + 16 * 20, BUTTON_WIDTH, FONT_NORMAL_BLACK);

    graphics_reset_dialog();
}

static void on_scroll(void)
{
    window_request_refresh();
}

static void handle_input(const mouse *m, const hotkeys *h)
{
    const mouse *m_dialog = mouse_in_dialog(m);
    if (generic_buttons_handle_mouse(m_dialog, 0, 0, buttons, MAX_BUTTONS, &data.focus_button_id)) {
        return;
    }
    if (input_go_back_requested(m, h)) {
        window_go_back();
    }
}

static void set_amount_param1(int value)
{
    data.action->parameter1 = value;
}

static void set_amount_param2(int value)
{
    data.action->parameter2 = value;
}

static void set_amount_param3(int value)
{
    data.action->parameter3 = value;
}

static void set_amount_param4(int value)
{
    data.action->parameter4 = value;
}

static void set_amount_param5(int value)
{
    data.action->parameter5 = value;
}


static void button_amount(int param1, int param2)
{
    if (param1 == 1) {
        window_numeric_input_show(screen_dialog_offset_x() + 60, screen_dialog_offset_y() + 50, 9, data.xml_info->xml_parm1.max_limit, set_amount_param1);
    } else if (param1 == 2) {
        window_numeric_input_show(screen_dialog_offset_x() + 60, screen_dialog_offset_y() + 50, 9, data.xml_info->xml_parm2.max_limit, set_amount_param2);
    } else if (param1 == 3) {
        window_numeric_input_show(screen_dialog_offset_x() + 60, screen_dialog_offset_y() + 50, 9, data.xml_info->xml_parm3.max_limit, set_amount_param3);
    } else if (param1 == 4) {
        window_numeric_input_show(screen_dialog_offset_x() + 60, screen_dialog_offset_y() + 50, 9, data.xml_info->xml_parm4.max_limit, set_amount_param4);
    } else if (param1 == 5) {
        window_numeric_input_show(screen_dialog_offset_x() + 60, screen_dialog_offset_y() + 50, 9, data.xml_info->xml_parm5.max_limit, set_amount_param5);
    }
}

void window_editor_scenario_action_edit_show(scenario_action_t *action)
{
    window_type window = {
        WINDOW_EDITOR_SCENARIO_EVENTS,
        draw_background,
        draw_foreground,
        handle_input
    };
    init(action);
    window_show(&window);
}
