...
static const uint8_t *get_display_string(special_attribute_mapping_t *attribute);
...
static void init(parameter_type type, void (*callback)(int), int current_value)
{
    ...
    switch (type) {
        case PARAMETER_TYPE_CHECK:
            data.list_size = SPECIAL_ATTRIBUTE_MAPPINGS_CHECK_SIZE;
            data.mapping_list = special_attribute_mappings_check;
            break;
        case PARAMETER_TYPE_DIFFICULTY:
            data.list_size = SPECIAL_ATTRIBUTE_MAPPINGS_CHECK_DIFFICULTY;
            data.mapping_list = special_attribute_mappings_difficulty;
            break;
        case PARAMETER_TYPE_BOOLEAN:
            data.list_size = SPECIAL_ATTRIBUTE_MAPPINGS_BOOLEAN_SIZE;
            data.mapping_list = special_attribute_mappings_boolean;
            break;
        case PARAMETER_TYPE_POP_CLASS:
            data.list_size = SPECIAL_ATTRIBUTE_MAPPINGS_POP_CLASS_SIZE;
            data.mapping_list = special_attribute_mappings_pop_class;
            break;
        case PARAMETER_TYPE_BUILDING:
        case PARAMETER_TYPE_BUILDING_COUNTING:
            data.list_size = BUILDING_TYPE_MAX;
            data.mapping_list = special_attribute_mappings_buildings;
            break;
        case PARAMETER_TYPE_ALLOWED_BUILDING:
            data.list_size = SPECIAL_ATTRIBUTE_MAPPINGS_ALLOWED_BUILDINGS_SIZE;
            data.mapping_list = special_attribute_mappings_allowed_buildings;
            break;
        case PARAMETER_TYPE_STANDARD_MESSAGE:
            data.list_size = SPECIAL_ATTRIBUTE_MAPPINGS_STANDARD_MESSAGE_SIZE;
            data.mapping_list = special_attribute_mappings_standard_message;
            break;
        default:
            window_go_back();
            return;
...
static void set_heading_display_text(void)
{
    special_attribute_mapping_t *mapping = scenario_events_parameter_data_get_attribute_mapping_by_type_and_value(data.parameter_type, data.current_value);
...
static void populate_list(int offset)
{
    ...
    for (int i = 0; i < MAX_VISIBLE_ROWS; i++) {
        int target_type = i + offset;
        if (target_type < data.list_size) {
            data.list[i] = &data.mapping_list[target_type];
        }
    }
}
