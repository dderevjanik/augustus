...
type = difficulty
check = What sort of check are we doing. Allowed values: "eq", "gte", "lte".
value = The difficulty to check for. Allowed values: "very_easy","easy","normal","hard","very_hard" (or values 0 to 4)
...
type = request_immediately_start
        ...
        + Note: Will not do anything if that specific request is already active.
                (i.e. player has received the request but not fulfilled or ignored it yet.)
        + Note: If you use this to start a request earlier than it is setup for,
                it will no longer run at its predefined time. (This action basically overwrites the request to start immediately, and start over if it has been fulfilled before.)
